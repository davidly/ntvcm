...
The app supports tracing to the ntvcm.log file. Instruction tracing includes a disassembler that shows instructions either as 8080 or Z80 depending on the mode. The `-g:<file>` option writes a per-PC execution-count profile as CSV at exit, with `pc,count,asm` columns.

Helper scripts live in the `scripts` directory. They use only the Python standard library.

    python3 scripts/profile_summary.py profile.csv --top 40
    python3 scripts/profile_diff.py before.csv after.csv --top 40
    python3 scripts/bench_ntvcm.py --repeat 10 --warmup 2 -- ./ntvcm -p test.com
    python3 scripts/bench_ntvcm.py --repeat 5 --profile-template /tmp/ntvcm-{run}.csv -- ./ntvcm test.com

`profile_summary.py` summarizes hot PCs and address ranges from an `ntvcm -g` CSV. `profile_diff.py` compares two profile CSVs and reports the biggest per-PC increases and decreases. `bench_ntvcm.py` repeats an ntvcm command, reports timing statistics, extracts cycle counts from `-p` output when present, and checks that repeated runs produce stable output.
...
The two versions of Turbo Pascal and the apps they generate use a tiny fraction of Z80 instructions. It took a day to implement the instructions for Turbo Pascal, and three more days to implement the full instruction set. Getting the undocumented features to work took an extra couple days. The invaluable resources required to do that are listed in x80.cxx. No single resource was 100% correct or complete. It takes a village :)
    ...
    Usage: ntvcm [-?] [-c] [-g:<file>] [-p] [-s:X] [-t] <command> [arg1] [arg2]
    A CP/M 2.2 emulator.
    ...
    -g:<file> write a per-PC execution-count profile CSV at exit.
